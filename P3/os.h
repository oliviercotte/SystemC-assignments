...
public:
	os_task();
	~os_task();
	void instantiate(sc_core::sc_process_handle handle, const char *name,
...
public:
	...
	os_task *task_create(const char *name, unsigned int priority,
...
private:
	...
	os_task *blocked_writer, *blocked_reader;

public:
	os_fifo(class os *host_os, const char *name, int size, sc_time wrd =
			sc_time(0, SC_MS), sc_time rdd = sc_time(0, SC_MS)) :
			sc_fifo<T>(name, size) {
		wr_delay = wrd;
		rd_delay = rdd;
		blocked_writer = NULL;
...
// We assume that there is only one writer task per FIFO
	void write(const T& data) {
		...
		if (blocked_reader && blocked_reader->get_state() == SUSPENDED) {
			...
			if (blocked_reader->wr_suspended_flag == false) {
				...
				if (blocked_reader->get_priority() > temp_t->get_priority()) {
...
// We assume that there is only one reader task per FIFO
	void read(T& data) {
		...
		if (blocked_writer && blocked_writer->get_state() == SUSPENDED) {
			...
			if (blocked_writer->rd_suspended_flag == false) {
				...
				if (blocked_writer->get_priority() > temp_t->get_priority()) {
								temp_t->task_state = READY;
								OS->READY_TASKS.push_back(temp_t);
								OS->schedule();
								blocked_writer = 0;
